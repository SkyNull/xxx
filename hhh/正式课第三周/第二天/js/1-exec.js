...
console.log(reg2.exec(str2));//2018 2019
console.log(reg2.exec(str2));//2019 null
console.log(reg2.exec(str2));//null 2018
console.log(reg2.exec(str2));//2018 2019
//reg2.lastIndex ---> 记录上次查找结束的索引，也就是下一次开始查找的索引
//test和exec  都会改变正则的lastIndex值
//通过一个方法 一次性拿到全部
RegExp.prototype.execAll = function (str) {
    var temp = null;
    if(!this.global){// 判断是否缺少 全局修饰符；若缺少的话直接返回匹配的第一个字符或字符串；然后加上自己的错误信息
        /*
        * 在这里可以新造一个正则；适用this + g 拼接的一个新正则
        *
        * */
        temp = this.exec(str) || {};
