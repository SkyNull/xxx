...
//fs.readFile("./custom.json","utf8",function (err,data) {
   /* console.log(err);
    console.log(data);*/
//});
// 2. readFileSync : 是一个同步读取文件的方法；会把读取的内容给当前的返回值
// readFileSync(path,option)
// 读出来的数据默认是一个JSON格式的字符串；JSON.parse转换成JSON对象;
...
// 当异步写入成功之后，执行这个回调；新的数据会将老的数据进行覆盖；以下方法是解决这个添加会覆盖的问题的；
fs.readFile("./custom.json","utf8",function (err,data) {
...
// writeFileSync : 这个方法没有return返回值，默认返回值是undefined
